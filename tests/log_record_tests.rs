...
fn parse_log_server_start() {
    let expected = LogRecord::new(
        ...
        "Done (4.736s)! For help, type \"help\"".to_string(),
    );
...
fn parse_log_server_version() {
    let expected = LogRecord::new(
        ...
        "Starting minecraft server version 1.19.3".to_string(),
    );
...
fn parse_log_server_overload() {
    let expected = LogRecord::new(
        ...
        "Can't keep up! Is the server overloaded? Running 2033ms or 40 ticks behind".to_string(),
    );
...
fn parse_log_user_auth() {
    let expected = LogRecord::new(
        ...
        LogLevel::Info,
        LogClass::UserAuth,
        "UUID of player Player is 9zz9999z-9zzz-99zz-z9zz-9z9z9zzzz99z".to_string(),
    );
